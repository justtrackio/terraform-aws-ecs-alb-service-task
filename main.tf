locals {
  ...
  enable_ecs_service_role         = module.this.enabled && var.network_mode != "awsvpc" && length(var.ecs_load_balancers) >= 1 && var.ecs_service_role_enabled
...
module "service_connect_label" {
  ...
  label_order = var.label_orders.service_connect
